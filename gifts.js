module.exports = [

		{name: `Flatiron Pepper Co "I Can't Feel My Face" Pepper Flakes`, 
			image: "./images/flatironpepper.jpg", 
			link: "https://www.flatironpepper.com/collections/frontpage/products/i-cant-feel-my-face",
			sortPrice: 20,
			listPrice: "$20 + s&h",
			quote: "What can I say? I like some spice with my food."},
		{name: `Lace Lab Red Flat Shoe Laces (63")`, 
			image: "./images/lacelabred.jpg", 
			link: "https://www.lacelab.com/collections/flat-laces/products/red-shoe-laces?variant=7899986693",
			sortPrice: 5,
			listPrice: "$5 + s&h",
			quote: "Sometimes it's nice to mix things up with a custom option."},
		{name: `Dinner at Sushi Taku`, 
			image: "./images/sushitaku.jpg", 
			link: "http://www.sushitaku.com",
			sortPrice: 36,
			listPrice: "~$36/person (after tax & tip)",
			quote: "I'd eat here every night if I could afford that much mercury."},
		{name: `Pyritized Ammonite`, 
			image: "./images/ammonite.jpg", 
			link: "https://davesrockshop.com/ammonite-plate-with-stand-i-fsl-1422.html",
			sortPrice: 39.5,
			listPrice: "$39.50",
			quote: "It's a fool's gold fossil. How awesome is that?"},
		{name: `Stance Socks Star Wars Duos 4 Pack (Large)`, 
			image: "./images/stancesw.jpg", 
			link: "https://www.ebay.com/itm/Stance-NEW-Unisex-Star-Wars-Duos-4-Pack-Socks-Multi-BNWT/254399781541?hash=item3b3b689aa5:m:mr8LgANaxH8nVV3RC1_QxOQ&var=553998741878",
			sortPrice: 85,
			listPrice: "~$85 + s&h",
			quote: "You wouldn't have to Force me to put a pair of these on my feet."},
		{name: `Stainless Steel Liquor Pour Spouts`, 
			image: "./images/spouts.jpg", 
			link: "https://www.amazon.com/Liquor-Pour-Spouts-Set-Stainless/dp/B01LW2L7R9",
			scrape: true,
			quote: "Take two cocktail-making classes and suddenly you're everyone's bartender..."},
		{name: `Buffalo Wild Wings Mango Habanero Sauce`, 
			image: "./images/bwwmango.jpg", 
			link: "https://www.amazon.com/Buffalo-Wild-Wings-Sauce-Habanero/dp/B00D4XNNCM",
			scrape: true,
			quote: "Everything tastes better with mango habanero!"},
		{name: `KitchenAid Pasta Roller Set`, 
			image: "./images/roller.jpg", 
			link: "https://www.amazon.com/KitchenAid-KSMPRA-3-Piece-Roller-Attachment/dp/B01DBGQR1K",
			scrape: true,
			quote: "Give a man pasta and he'll eat for a day. Get him this pasta roller and he'll never set foot in an Olive Garden again."},
		{name: `Air Max 1 Tinker Sketch to Shelf Black (Size 10)`, 
			image: "./images/airmaxtinker.jpg", 
			link: "https://stockx.com/nike-air-max-1-tinker-sketch-to-shelf-black",
			size: 10,
			scrape: true,
			quote: 	`They're like wearing an episode of "How It's Made".`},
		{name: `Jordan 1 Retro Chicago (Size 9.5)`, 
			image: "./images/ajchi.jpg", 
			link: "https://stockx.com/jordan-1-retro-chicago-2015",
			size: 9.5,
			scrape: true,
			quote: "Simple. Clean. Classic."},
		{name: `Carlisle CC Rack & Pinion Mount Flameworking Torch`, 
			image: "./images/carlislecc.jpg", 
			link: "https://www.mountainglass.com/Carlisle-CC-Rack-Pinion-Mount-4-Inlet-5-Centerfire",
			sortPrice: 1635,
			listPrice: "$1635 + s&h",
			quote: `The most important tool I'm missing to create more art like <a href="https://alex.berez.com/glass" target="blank"">this</a>.`},
		{name: `LEGO Imperial Star Destroyer`, 
			image: "./images/destroyer.jpg", 
			link: "https://www.lego.com/en-us/product/imperial-star-destroyer-75252",
			sortPrice: 700,
			listPrice: "$700",
			quote: "Yes, it's almost 4 feet long. No, I don't have anywhere to put it. Why do you ask?"},
		{name: `Rishadan Port Print`, 
			image: "./images/port.jpg", 
			link: "https://www.ebay.com/itm/LIMITED-EDITION-Signed-MTG-Magic-Mercadian-Masques-Rishadan-Port-Print-Giclee/392799762281",
			sortPrice: 70,
			listPrice: "$70 + s&h",
			quote: "One of my favorite pieces of Magic: the Gathering art."},
		{name: `Dark Ritual Print`, 
			image: "./images/darkrit.jpg", 
			link: "https://rebeccaguaymtgartstore.weebly.com/store/p60/Dark_Ritual.html",
			sortPrice: 90,
			listPrice: "$90",
			quote: "An iconic artist's take on an iconic Magic: the Gathering card."},
		{name: `Brandon Sanderson Leather-bound Book (Warbreaker, Well of Ascension, or The Hero of Ages)`, 
			image: "./images/leatherbook.jpg", 
			link: "https://store.brandonsanderson.com/collections/signed-leatherbound-books",
			sortPrice: 100,
			listPrice: "$100 + s&h",
			quote: "Filling my library with fancy leather books, starting with the works of my favorite author."},
		{name: `Canadian $4 Fossilized Tyrannosaurus Rex Silver Coin`, 
			image: "./images/silvertrex.jpg", 
			link: "http://www.londoncoincentreinc.com/canada-2009-4-tyrannosaurus-rex-dinosaur-skeleton-pure-silver-coin/",
			sortPrice: 42,
			listPrice: "$42 + s&h",
			quote: "Precious metals and dinosaurs: a timeless combination."},
		{name: `Marabou Daim Chocolate Bar`, 
			image: "./images/maraboudaim.jpg", 
			link: "https://www.amazon.com/Marabou-Daim-Milk-Chocolate-King/dp/B00642BRGY",
			scrape: true,
			quote: "The secret ingredient in the best chocolate chip cookies you'll ever taste."},
		{name: `Nike Adapt BB 2.0 Mag (Size 10)`, 
			image: "./images/mag.jpg", 
			link: "https://stockx.com/nike-adapt-bb-2-mag-us-charger",
			size: 10,
			scrape: true,
			quote: "The closest thing to a functional pair of <i>Back to the Future II</i>'s Nike Air Mags this side of $30,000."},
		{name: `Wave Runner Slippers`, 
			image: "./images/waveslippers.jpg", 
			link: "https://bannedgoods.com/collections/view-all/products/wave",
			sortPrice: 45,
			listPrice: "$45 + s&h",
			quote: "Not your average pair of slippers."},
		{name: `Copper Gnome Cocktail Shaker`, 
			image: "./images/gnomeshaker.jpg", 
			link: "https://www.williams-sonoma.com/products/elyx-copper-gnome-cocktail-shaker/?pkey=ctabletop-bar-tool%7Cbar-cocktail-shaker",
			sortPrice: 70,
			listPrice: "$70",
			quote: "The best way to make drinks is in a high-end lawn ornament."},
		{name: `The Refined Feline Lotus Cat Tower`, 
			image: "./images/cattower.jpg", 
			link: "https://www.amazon.com/Refined-Feline-Lotus-Tower-Espresso/dp/B004BA6LSK",
			scrape: true,
			quote: "This one's not actually for me. Honest."},
		{name: `Parrot Anafi FPV Drone Set`, 
			image: "./images/parrotdrone.jpg", 
			link: "https://www.amazon.com/dp/B07WHDGR2R",
			scrape: true,
			quote: "Practice social distancing via remote exploration."},
		{name: `Color Changing Smart Bulbs`, 
			image: "./images/smartbulb.jpg", 
			link: "https://www.amazon.com/Changing-Compatible-Assistant-Multicolor-LUMIMAN/dp/B07DLSNNDS",
			scrape: true,
			quote: "Physically controlling lights is so last century."},
		{name: `Mythosaur Skull`, 
			image: "./images/mythosaur.jpg", 
			link: "https://regalrobot.com/shop/star-wars/starwars-art-sculpture-decor/mythosaur-skull-mandalorian-wall-decor/",
			sortPrice: 275,
			listPrice: "$275 + s&h",
			quote: "Someday, I will have a fireplace. Above that fireplace: this."},
		{name: `Satisfye Slim Grip Case for Nintendo Switch`, 
			image: "./images/satisfyecase.jpg", 
			link: "https://www.satisfye.com/collections/stuck-at-home/products/slim-grip-case",
			sortPrice: 26,
			listPrice: "$26",
			quote: `There's no such thing as "too many gaming accessories".`},
		{name: `Honeycomb Pan`, 
			image: "./images/beepan.jpg", 
			link: "https://www.nordicware.com/honeycomb-pull-apart-pan",
			sortPrice: 52,
			listPrice: "$52",
			quote: "Bee cakes!"},
		{name: `Rocket Bookends`, 
			image: "./images/rocketbookend.jpg", 
			link: "https://www.wayfair.com/decor-pillows/pdp/breakwater-bay-rocket-non-skid-bookends-bkwt7943.html",
			sortPrice: 124,
			listPrice: "$124",
			quote: "A great way to display my sci-fi and fantasy books."},
		{name: `adidas Consortium Runner V2 4D (Size 9.5)`, 
			image: "./images/4drunner.jpg", 
			link: "https://stockx.com/adidas-consortium-runner-v2-4d-collegiate-navy",
			size: 9.5,
			scrape: true,
			quote: "3D printed shoes. The future is now!"},
		{name: `Leather-bound Foundation Trilogy`, 
			image: "./images/asimov.jpg", 
			link: "https://www.eastonpress.com/deluxe-editions/isaac-asimovs-the-foundation-trilogy-a-signed-edition-3230.html",
			sortPrice: 395,
			listPrice: "$395",
			quote: "A timeless classic given a deluxe upgrade."},
		{name: `Giant Skeleton`, 
			image: "./images/skeleton.jpg", 
			link: "https://www.homedepot.com/p/Home-Accents-Holiday-12-ft-Giant-Sized-Skeleton-with-LifeEyes-5124738/312513260",
			sortPrice: 300,
			listPrice: "$300",
			quote: "The perfect gift for the most important holiday of the year."},
	]

/* Gift Templates

Scrape Amazon: 

		{name: ``, 
			image: "./images/", 
			link: "",
			scrape: true,
			quote: ""},

Scrape StockX: 

		{name: ``, 
			image: "./images/", 
			link: "",
			size: ,
			scrape: true,
			quote: ""},

No Scrape: 

		{name: ``, 
			image: "./images/", 
			link: "",
			sortPrice: ,
			listPrice: "",
			quote: ""},

*/